...
Use this to assign each new booking to the next available teammate—fair load, faster response times, no manual routing. Helpful for sales, support, clinics, and any team sharing the same event. 
...
When to hide "Event Types" vs show them (member-first vs service-first)

Hide "Event Types", show members (booker picks a person): Use when clients know whom they want (e.g., book directly with a doctor/coach). Round robin still balances internally when links route to a shared event.

Show "Event Types", hide most members (booker picks a service): Use when the service matters more than the person (e.g., “Product Demo – 30 min”). Round robin assigns the next available host automatically.
