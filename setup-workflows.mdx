...
After saving, perform a real or test booking that matches the trigger conditions (for example, book a meeting or cancel one). Check that the email or SMS messages are sent as expected to all recipients. Adjust timing, variables, or recipients if needed.
